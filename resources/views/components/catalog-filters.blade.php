...
<div class="relative font-serif filter-component-wrapper">
    ...
    <aside class="filter-sidebar relative fixed inset-y-0 right-0 z-50 w-[320px] bg-black shadow-2xl transform transition-transform duration-300 translate-x-full 
        ...
        <div class="hidden lg:block pb-5 border-b border-gray-800 mb-4">
            <span class="text-lg font-bold text-white capitalize tracking-widest flex items-center gap-2">
...
<script>
    ...
    function setRange(element, type, min, max) {
        ...
        const activeBorderClass = 'border-[#cd1d46]';
        siblings.forEach(el => {
            el.querySelector('.radio-circle').classList.remove(activeBorderClass);
        ...
        if (!isReset) {
            element.querySelector('.radio-circle').classList.add(activeBorderClass);
